...
/* -------------------------------------------------------------------------- */
  ...
  TestResult
test_VInfo_constructorsRegistry()
{
  try
    {
      floco::db::VInfo p0(
      ...
      floco::db::VInfo p1( "lodash", "4.17.21" );
      if ( p0 != p1 )
        {
          return TestResult(
            "VInfo constructors registry URLs produced different results"
          );
        }
    }
  catch( ... )
    {
      return TestResult(
        test_status::error
      , "ERROR: Failed to construct VInfo(s) from registry fetcher(s)"
      );
    }
  return TestResult();
}
...
main( int argc, char * argv[], char ** envp )
{
  ...
  rsl = test_VInfo_constructorsRegistry();
