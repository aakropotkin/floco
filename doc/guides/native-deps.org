...
cp ../zeromq-5.3.1/{default,floco-cfg}.nix .;
...
Now we'll ramp up the difficulty by trying to build a later releas of
=zeromq@6.0.0-beta.16= with a shared library pulled from =nixpkgs=.

I am writing this guide without having packaged this before so for all we
know this might not work because this project is a beta release or a
compatability issue with the =nixpkgs= libs; but this is probably a good
...
varialbe =zmq_shared= ( among others ) which has a default value of
=false=, and that when building on Darwin with ~zmq_shared = true~, it
will use =pkg-config= to locate =libsodium=.
